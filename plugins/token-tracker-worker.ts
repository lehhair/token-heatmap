...
async function main() {
  if (!process.argv[1]?.replace(/\\/g, "/").endsWith("/token-tracker-worker.ts")) return
  try {
    await syncData()
    ;(globalThis as any).postMessage?.({ type: "done" })
  } catch (error: any) {
    const message = error?.message || String(error)
    ;(globalThis as any).postMessage?.({ type: "error", message })
    console.error(`[token-tracker] sync failed: ${message}`)
  }
}
